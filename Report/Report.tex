...
\begin{document}
    ...
    \section{PDE definition and methods}\label{secModelDef}
        ...
        \subsection{Proper Orthogonal Decomposition (POD)}\label{secPOD}
            ...
            Considering a parametrized partial differential equation whose solution lies in the functional space $\mathbb{V}^\boldmu$, POD is an algorithm for the generation of the reduced basis based on two different stages:
            ...
            In the POD method, to find the basis for our reduced space, we need to define the correlation matrix $\mathbf{C} \in \mathbb{R}^{M \times M}$. First, we select $M$ solutions $\omega_m=u^\boldmu_\delta(\boldmu_m) \in \mathbb{V}_{\delta}$ from the high fidelity problem. Then we solve the eigenvalue problem:
            %
            \begin{equation}
                \label{eq:eig}
        ...
        \subsection{Physics-Informed Neural Networks (PINN)}\label{secPINN}
            ...
            \begin{equation}
                \label{eq:NN}
                \begin{aligned}
                    \mathcal{F}_{NN} \colon \quad & \Omega \times \mathcal{P} \to \mathbb{V}^\boldmu, \\
                    & (x,\boldmu) \mapsto \hat{u}(x,\boldmu) \approx u(x,\boldmu).
                \end{aligned}
            \end{equation}
            %
            where $u(x,\boldmu)$ is the exact solution of the PDE and $\mathbb{V}^\boldmu$ is the functional space where the solution lies. 
            ...
            Given the neural network $v(x,\boldmu) \in \mathcal{F}_{NN}$, the loss function is defined as:
            %
            \begin{equation}
                \MSE=\MSE^\boldmu_b+\lambda \MSE^\boldmu_p
            \end{equation}
            %
            where $\MSE^\boldmu_b$ is the boundary loss, $\MSE^\boldmu_p$ is the physical loss and $\lambda$ is the hyperparameter that balances the tradeoff between the two contributions, to be set before the network training.
            
            In particular, if the physical system we are studying is described by a PDE of the form $\mathcal R(u(x,\boldmu))=0$, the errors are given by:
            %
            \begin{equation*}
                \MSE^\boldmu_b=\frac{1}{N_b} \sum_{k=1}^{N_b} | v(x^b_k,\boldmu_k^b) - u(x_k^b,\boldmu_k^b)|^2,
            \end{equation*} 
            %       
            \begin{equation*}
               \MSE^\boldmu_{p}=\frac{1}{N_p}\sum_{k=1}^{N_p}|\mathcal R(v(x^p_k,\boldmu_k^p))|^2
            \end{equation*}
            %
            for $(x^b_k,\boldmu_k^b) \in (\partial \Omega, \mathcal{P})$ and $(x^p_k,\boldmu_k^p) \in (\Omega, \mathcal{P})$.
            
            Finally, the solution approximation is given by
            %
            \begin{equation}
                \hat{u}(x,\boldmu):= \operatorname*{argmin}_{v \in \mathcal{F}_{NN}} \MSE.
            \end{equation}
